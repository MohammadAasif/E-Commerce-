...
async function main() {
    ...
    //Insert the data
    const insertData = await db.collection("Role_0").insertMany(data);
    console.log(insertData.insertedCount + "record inserted");

    //Display the data
    const getusers = await db.collection("Role_0").find().toArray();
    console.log(getusers);

    //Updat a data
    query = { Name: "Yash" };
    const updatequery = { $set: { Slug: "https://yash.doodle.com/sample/" } };
    ...
    const upusers = await db.collection("Role_0").find().toArray();
    console.log(upusers);
//Delete the data 
    const deleteUser = await db.collection("Role_0").deleteMany();
